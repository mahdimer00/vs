...
export function DirectOrderForm({ product, variant, quantity, shippingFee: initialFee, importantNotes = [] }: DirectOrderFormProps) {
  ...
  const submit = async (opts: { manualConfirmOverride?: boolean; token?: string | null } = {}) => {
    // Hard guard — never allow re-entry once submitted or while submitting
    if (submitting || orderSubmitted) return;
    ...
    try {
      ...
      // Lock permanently BEFORE navigating — setSubmitting(false) must NOT run on success
      setOrderSubmitted(true);
      navigate("/order/success");
      // intentionally no setSubmitting(false) here — form stays locked
    } catch (error) {
      const message = error instanceof Error ? error.message : translate(language, "checkoutSubmitting");
      setErrorMessage(message);
      pushToast(message, "error");
      setSubmitting(false); // only reset on failure so they can retry
    }
  };
